...
for i in range (0, countTraining): #4
	...
	for j in range (0, countSetting): #7
		...
		if j==3:
			plt.bar(r4, vTime, width=barWidth, edgecolor='black', label = "parallel")
			plt.errorbar(r4, vTime, vStdDev, marker='^', linestyle='None', color='black')
			plt.xticks(r4, vThreads, rotation ='horizontal')

		if j==4:
			plt.bar(r5, vTime, width=barWidth, edgecolor='black', label = "paravx")
			plt.errorbar(r5, vTime, vStdDev, marker='^', linestyle='None', color='black')
			plt.xticks(r5, vThreads, rotation ='horizontal')	

		if j==5:
			plt.bar(r6, vTime, width=barWidth, edgecolor='black', label = "parsimd")
			plt.errorbar(r6, vTime, vStdDev, marker='^', linestyle='None', color='black')
			plt.xticks(r6, vThreads, rotation ='horizontal')

		#if j==6:
		#	plt.bar(r7, vTime, width=barWidth, edgecolor='black', label = "parseq")
		#	plt.errorbar(r7, vTime, vStdDev, marker='^', linestyle='None', color='black')
