...
//static unsigned long distCounter = 0;
...
#pragma omp parallel for \
  private(q, tmp, weight, opf_DistanceValue) \
  shared(lockvar, Q, p, sg, i, pathval, opf_PrecomputedDistance, opf_ArcWeight) \
  default(none)
#endif
...
void opf_OPFLearning(Subgraph **sgtrain, Subgraph **sgeval){
	...
	int ignoreDelta = 0, writeIterationsFile = 0;
	char *iterations_env_var = getenv("NITERATIONS");
	if (iterations_env_var != NULL) {
		int niter = atoi(iterations_env_var);
		if ( niter == 0 ) {
			writeIterationsFile = 1;
		} else {
			ignoreDelta = 1;
			iterations = niter;
		}
  ...
  for (i=0; i < iterations; i++) {
    ...
    //fprintf(stdout, "counter = %lu\n", distCounter); 
		...
		if ( !ignoreDelta && (delta <= 0.0001) ) break;
	}
	if ( writeIterationsFile ) {
		FILE *f = fopen("iterations", "w");
		if ( f == NULL ){
			perror("fopen");
			fprintf(stderr, "error: could not create iterations file!\n");
			exit(EXIT_FAILURE);
		}
		fprintf(f,"%d\n",i+1);
		fclose(f);
...
#ifdef PARALLEL
# pragma omp parallel for \
  private(q, weight, opf_DistanceValue) \
  shared(lockvar, Q, p, sg, pathval, opf_PrecomputedDistance, opf_ArcWeight) \
  default(none)
#endif
    for (q=0; q < sg->nnodes; q++){
      if (Q->color[q]!=BLACK){
...
//float opf_EuclDist(afloat * __restrict__ f1, afloat * __restrict__ f2, int n){
...
#ifdef VECT_AVX
  __m256 R_f1, R_f2;
  __m256 R_acc = _mm256_setzero_ps();

  for (i=0; i < n-8; i+=8){
    R_f1 = _mm256_load_ps(&f1[i]);
    R_f2 = _mm256_load_ps(&f2[i]);
    R_acc = R_acc + ((R_f1 - R_f2) * (R_f1 - R_f2));
  }
 
  __m256 tmp = _mm256_permute2f128_ps(R_acc,R_acc, 0x1);
  R_acc = _mm256_add_ps(R_acc, tmp);
  R_acc = _mm256_hadd_ps(R_acc, R_acc);
  R_acc = _mm256_hadd_ps(R_acc, R_acc);

  float tmp2[8] __attribute__((aligned(32)));
  _mm256_store_ps(tmp2, R_acc);
  dist += tmp2[0];
